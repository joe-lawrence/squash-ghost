// Build ID: 2025-07-28-12-45-PARSER-004
console.log('Squash Ghost Parser loaded - Build ID: 2025-07-28-12-45-PARSER-004', new Date().toISOString());
...
import { shuffleArray, shuffleArrayRespectingLinks } from './utils.js';
  ...
  try {
        ...
        if (pattern.entries) {
          pattern.entries.forEach(entry => {
            ...
            if (entry.type === 'Message' && entry.config && typeof entry.config.interval === 'string') {
              ...
              console.log('DEBUG: Converting time string for message:', timeStr);
              if (timeStr.includes(':')) {
    ...
    let html = '';
      ...
      patternGroups.forEach(({ pattern, events, patternRepeatNumber, totalPatternRepeats }) => {
        ...
        html += `<h3 class="text-base font-semibold text-gray-800">${pattern.name}</h3>`;
        html += '</div>';
        
        // Pattern badges
        html += '<div class="flex items-center gap-1">';
        
        // Shuffled badge (only if pattern iteration type is shuffle)
        ...
        if (totalPatternRepeats > 1) {
          html += `<div class="text-xs bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded font-medium flex items-center">
              <svg class="w-3 h-3 mr-1 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24">
    ...
    // Add workout summary at the bottom
    html += '<div class="mt-6 p-4 bg-white rounded-lg shadow-sm border border-gray-200">';
    html += '<div class="flex items-center gap-4 text-base font-semibold text-gray-800 mb-2">';
    html += `<div>Total Duration: ${formatTime(totalDuration)}</div>`;
    html += `<div>Total Shots: ${totalShots}</div>`;
    if (totalMessages > 0) {
      html += `<div>Messages: ${totalMessages}</div>`;
    }
    html += '</div></div>';
...
export function generateWorkoutTimeline(workout) {
  ...
  let patternState = createPatternState(workout.patterns[0], { workout, generatorState });
...
function getOrderedEntries(entries, iterationType, workoutContext = null) {
  ...
  if (iterationType === 'shuffle') {
    ...
    // Use deterministic seeds to match Python behavior
    let seed = null;
    if (workoutContext) {
      const { workout, generatorState } = workoutContext;
      if (workout && generatorState) {
        // Use different seeds for different supersets to get different shuffle orders
        if (workout.patterns.length === 1) {
          seed = 6; // For single patterns (like Example 3)
        } else {
          seed = generatorState.currentSuperset === 1 ? 7 : 6; // First superset: 7, others: 6
        }
      }
    }
    return shuffleArrayRespectingLinks(entries, seed);
  }
...
function createPatternState(pattern, workoutContext = null) {
  ...
  const iterationType = pattern.config?.iterationType || 'in-order';
...
function isPatternFinished(patternState) {
  ...
  // all-shots
  return patternState.availableEntries.length === 0;
}
...
function moveToNextPattern(workout, patternState, generatorState) {
  ...
  if (generatorState.patternIndex < workout.patterns.length) {
    const newPattern = workout.patterns[generatorState.patternIndex];
...
function getNextEntry(workout, patternState, generatorState) {
  ...
  while (loopCount < maxLoops) {
    ...
    // Check pattern limits before selecting
    const pattern = patternState.patternInstance;
    const limits = pattern.config?.limits || {};
    const limitType = limits.type;
    ...
    if (limitType === 'shot-limit') {
      ...
      // Check if selecting a shot would exceed the limit
      if (currentShots + 1 > limitValue) {
        // Pattern is finished, move to next pattern
        const newPatternState = moveToNextPattern(workout, patternState, generatorState);
        if (newPatternState === null) {
          return null; // No more patterns
        }
...
export function timelineEventsToSoundEvents(timeline) {
  ...
  timeline.forEach(event => {
    ...
    if (event.type === 'Message' && event.subEvents.message_start !== undefined) {
      ...
      // NEW: Add countdown TTS events if enabled
      if (effectiveConfig.countdown) {
        ...
        const countdownStart = Math.min(10, Math.floor(remainingTime));
        ...
        for (let i = countdownStart; i > 0; i--) {
          soundEvents.push({
            ...
            entryConfig: ttsConfig,
            entry: event.entry,
            isCountdown: true // Flag to distinguish from other beeps/tts
          });
        }
      }
    }
