// Build ID: 2025-07-28-12-45-VALIDATION-004
console.log('Squash Ghost Validation loaded - Build ID: 2025-07-28-12-45-VALIDATION-004', new Date().toISOString());
...
export function validateBaseConfig(config) {
  ...
  if (config.shotAnnouncementLeadTime !== undefined) {
    if (
      ...
      config.shotAnnouncementLeadTime < 0
    ) {
      errors.push(
        new ValidationError({
          field: 'shotAnnouncementLeadTime',
          message: 'Shot announcement lead time must be a non-negative number',
