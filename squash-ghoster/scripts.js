...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Toggle between light and dark themes (short press)
  function toggleTheme() {
    toggleToOppositeMode();
    hideThemeDropdown();
  }

  // Show theme selector dropdown (long press mobile - shows opposite themes)
  function showThemeSelector() {
    const dropdown = document.getElementById('themeDropdown');
    const oppositeTheme = themeState.currentTheme === 'dark' ? 'light' : 'dark';

    populateThemeOptions(oppositeTheme);
  ...
  // Show current theme selector dropdown (desktop first click - shows current themes)
  function showCurrentThemeSelector() {
    const dropdown = document.getElementById('themeDropdown');
    const currentTheme = themeState.currentTheme;

    populateThemeOptions(currentTheme);
  ...
  // Populate theme options in dropdown
  function populateThemeOptions(themeType) {
    const container = document.getElementById('themeOptions');
    const schemes = themeSchemes[themeType];
    const currentScheme = themeType === 'dark' ? themeState.darkScheme : themeState.lightScheme;

    container.innerHTML = '';

    Object.entries(schemes).forEach(([schemeId, schemeData]) => {
      const option = document.createElement('button');
      option.className = `theme-option ${currentScheme === schemeId ? 'active' : ''}`;
      option.innerHTML = `
  ...
  // Toggle to opposite mode with memory
  function toggleToOppositeMode() {
    const oppositeTheme = themeState.currentTheme === 'dark' ? 'light' : 'dark';
    themeState.currentTheme = oppositeTheme;
    applyTheme();
  }
  ...
  if (isMobileDevice()) {
    // Mobile: differentiate between short and long press
    themeToggle.addEventListener("touchstart", (e) => {
      e.preventDefault();
      themeState.isLongPress = false;
      themeState.longPressTimer = setTimeout(() => {
        themeState.isLongPress = true;
        showThemeSelector();
    ...
    themeToggle.addEventListener("touchend", (e) => {
      e.preventDefault();
      clearTimeout(themeState.longPressTimer);
      if (!themeState.isLongPress) {
        toggleTheme();
      }
    });
    ...
    // Desktop: first click shows current theme options, second click switches modes
    themeToggle.addEventListener("click", (e) => {
      e.preventDefault();
      const dropdown = document.getElementById('themeDropdown');

      if (dropdown.classList.contains('show')) {
        // Second click: switch to opposite mode (with memory) and update dropdown
        toggleToOppositeMode();
        showThemeSelector(); // This will update the dropdown for the new current mode
      } else {
        // First click: show current theme options
        showCurrentThemeSelector();
      }
    });
  }
  ...
  function setToolbarIconsOpacity(opacity) {
    const toolbarButtons = document.querySelectorAll('.theme-toggle, .rocket-toggle, .toolbar-icon-btn');
  ...
  if (workoutReplayBtn) {
    workoutReplayBtn.addEventListener("click", function () {
      ...
      setTimeout(() => {
        console.log("Starting workout replay...");
  ...
  function startWorkoutExecution() {
    ...
    // Reset and initialize workout execution state (fresh start each time)
  ...
  function updateWorkoutUI() {
    ...
    if (isRunning && !isPaused) {
      statusIndicator.classList.add("running", "animate-pulse");
    } else if (isPaused) {
    ...
    if (workoutExecution.isCompleted) {
      // Workout is complete - show replay button only
      startBtn.classList.add("hidden");
      pauseBtn.classList.add("hidden");
      replayBtn.classList.remove("hidden");
    } else if (isRunning && !isPaused) {
      // Workout is running - show pause button only
      startBtn.classList.add("hidden");
      pauseBtn.classList.remove("hidden");
      replayBtn.classList.add("hidden");
    } else {
      // Workout is stopped or paused - show start button only
      startBtn.classList.remove("hidden");
      pauseBtn.classList.add("hidden");
      replayBtn.classList.add("hidden");

      // Update start button style based on state (no text since buttons are icon-only)
      if (isPaused) {
  ...
  function updateShotDisplay() {
    ...
    if (isPaused) {
      ...
      if (isTimeComplete || isLastShotComplete) {
        console.log("Workout complete! Time:", isTimeComplete, "LastShot:", isLastShotComplete);
        ...
        if (!workoutExecution.timelineIndex) {
          ...
          console.log('Built timeline index:', workoutExecution.timelineIndex);
        }
           ...
           if (timeSinceBeep <= pauseDuration) {
             ...
             if (timeSinceBeep <= 0.05 && !workoutExecution.glowTriggered) {
               ...
               if (shotTitleElement) {
                 ...
                 console.log("Shot text glow triggered at beep");
               ...
               if (nextShot) {
                 ...
                 if (remainingTime > 0) {
                   ...
                   if (currentTime >= shot.endTime && !workoutExecution.lastShotCompleted) {
                     ...
                     console.log("Last shot completed");
                   }
                 } else {
    ...
    // Debug output - only log when shotTitle changes or significant progress changes
    ...
    if ((shotTitle && shotTitle !== workoutExecution.lastShotTitle) || progressChanged) {
      console.log(`Display: "${shotTitle}", Type: ${activeEventType}, Progress: ${progressPercentage.toFixed(1)}%, Phase: ${progressPhase}`);
  ...
  function buildTimelineIndex(soundEvents) {
    ...
    console.log(`Timeline index: ${shots.length} shots, ${messages.length} messages`);
